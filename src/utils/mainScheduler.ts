...
const STORAGE_ID = "mainScheduler";
...
if (window.location.search === "?demo") {
  ...
  dataInput = localStorage.getItem(STORAGE_ID);
}

window.addEventListener("beforeunload", () => {
  localStorage.setItem(STORAGE_ID, new Scheduler(store.getState().app.students).toString());
});

export default new Scheduler(dataInput);
